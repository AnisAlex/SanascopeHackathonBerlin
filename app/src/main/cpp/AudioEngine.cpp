...
void AudioEngine::startThroughput() {
    record->resetRecord();
    record->resetReplay();
    inStream->start();
    outStream->start();
...
void AudioEngine::stopThroughput() {
    outStream->stop();
    inStream->stop();
}
...
void AudioEngine::startListening() {
    ...
    infoLog("Starting listening...");
    state = EngineState::Recording;
    cacheData = false;
    startThroughput();
    infoLog("Listening started.");
}
...
void AudioEngine::stopListening() {
    ...
    infoLog("Stopping listening...");
    state = EngineState::Idle;
    stopThroughput();
    infoLog("Listening stopped.");
}
...
oboe::DataCallbackResult AudioEngine::inputCallback(void* audioData, int32_t numFrames) {
    ...
    bool appended = record->appendFrames(data, numFrames);
    //infoLog("Audio data written.");

    processedFrameCount += numFrames;

    if (appended) {
        //infoLog("Added frames. Ready for more.");
        return oboe::DataCallbackResult::Continue;
    } else {
        errorLog("Buffer cannot take any more data!");
        return oboe::DataCallbackResult::Stop;
    }
}
